...
var resizePizzas = function(size) {
  ...
  /*
  This section was changed in order to obtain 60fps when resizing the image using the slider.
  The first thing was to remove everything unnecessary from the for loop which includes: Finding what size the slider
  position is on and setting a width using a switch statement for each size, and getting a list of pizzas that need to
  change size (randomPizzaContainer). Keeping this in the loop was causing unnecessary calculations.

  Upon obtaining this information, we iterate through each pizza to change the width.
   */
  function changePizzaSizes(size) {
    //find the size of the slider and set a width
    switch(size){
        case "1":
          newWidth = 25;
          break;
    ...
    //find all pizza containers in the document
    var randomPizzas = document.getElementsByClassName("randomPizzaContainer");
    //iterate through each object and change the width based on the slider case.
...
/* this section was also changed, randomPizzas only needs to be obtained once and not
continuously in a loop so it was moved outside.
 */
...
function updatePositions() {
  ...
  /*
  This section was changed in order to get 60fps in when scrolling up and down the site. Variables that were getting elements
  from the DOM were moved outside of the for loop as they only needed to be calculated once (items and scrolltop).

  Phase was also moved out as it only has 5 values for it and keeping it in the loop would be a waste of resource.
  This way phase gets calculated in its own loop from 0-4, and used in the second loop with mod 5
   */
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  /*in order to reduce the number of pizzas, the innherHeight is calculated to get the height of the view port.
  The value is divided by 200 to get the amount of rows, and multiplied by 8 as each row has 8 sliding pizzas.

  At the moment this part isn't responsive. If the height is small (minimized window) and then it is increased, the rows
  will not reflect this change
   */
  var numberOfPizzas = Math.floor(window.innerHeight/ 200) * 8;
